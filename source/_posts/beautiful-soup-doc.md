...
*   如果同样的代码在不同环境下结果不同,可能是因为两个环境下使用不同的解析器造成的.例如这个环境中安装了lxml,而另一个环境中只有html5lib, [解析器之间的区别](#id49) 中说明了原因.修复方法是在 `BeautifulSoup` 的构造方法中中指定解析器
...
| \[5\] |  采用先序遍历方式 |
| \[6\] |  CSS选择器是一种单独的文档搜索语法, 参考 [http://www.w3school.com.cn/css/css_selector_type.asp](http://www.w3school.com.cn/css/css_selector_type.asp) |
| \[7\] | 原文写的是 html5lib, 译者觉得这是原文档的一个笔误 |
| \[8\] | wrap含有包装,打包的意思,但是这里的包装不是在外部包装而是将当前tag的内部内容包装在一个tag里.包装原来内容的新tag依然在执行 [wrap()](#wrap) 方法的tag内 |
